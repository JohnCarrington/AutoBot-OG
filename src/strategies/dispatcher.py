...
def detect_all_setups(
    ...
    Each strategy is gated to a single regime, so the matrix is total
    and unambiguous — there is never more than one eligible strategy
    in v1.
    """
    current = regime_state.get("current_regime")
    if current == RegimeLabel.RANGE.value:
        result = detect_bb_reclaim(
            df_m5, df_h1, regime_state, pair, current_time
        )
